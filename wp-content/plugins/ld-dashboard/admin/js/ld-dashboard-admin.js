(function ($) {
  ...
  $(document).ready(function () {
    ...
    if ( jQuery('.use-admin-account-checkbox').length ) {
      if ( jQuery('.use-admin-account-checkbox').is(':checked') ) {
        ...
        jQuery('.ld-single-grid.ld-dashboard-instructors-listing').hide();
      }
      jQuery('.use-admin-account-checkbox').on('change', function(){
        if ( jQuery(this).is(':checked') ) {
          ...
          jQuery('.ld-single-grid.ld-dashboard-instructors-listing').hide();
          jQuery('.ld-single-grid.ld-dashboard-instructors-listing').find('.ld-grid-content').html('');
        }
      });
      function getCoHostsInstructorsList() {
        ...
        jQuery.ajax({
          ...
          data: params,
          success: function (response) {
            jQuery('.ld-single-grid.ld-dashboard-instructors-listing').find('.ld-grid-content').html(response);
            jQuery('.ld-single-grid.ld-dashboard-instructors-listing').show();
    ...
    if ( jQuery('.ld-dashboard-setting-accordian-wrapper').length ) {
      jQuery('.ld-dashboard-setting-accordian-wrapper').find( '.ld-dashboard-admin-title-section').on('click', function(){
    ...
    if ( jQuery('.ld-dashboard-menu-tab-checkbox-hidden').length ) {
      jQuery('.ld-dashboard-menu-tab-checkbox-hidden').each( function(){
        if ( jQuery(this).val() == 1 ) {
          jQuery(this).prev('.ld-dashboard-setting-switch').find('.ld-dashboard-menu-tab-checkbox').prop('checked', true);
        }
      });
      jQuery('.ld-dashboard-menu-tab-checkbox').on('change', function(){
        if ( jQuery(this).is(':checked') ) {
          jQuery(this).parent().next('.ld-dashboard-menu-tab-checkbox-hidden').val(1);
        } else {
          jQuery(this).parent().next('.ld-dashboard-menu-tab-checkbox-hidden').val(0);
        }
      });
    }
  ...
  $(function () {
    var acc_elmt = document.getElementsByClassName("ld-dashboard-accordion");
    var k;
